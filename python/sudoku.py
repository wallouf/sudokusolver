...
def loop_apply_strategies(values_readed):
    ...
    if recheck_all_results(values_readed):
...
def recheck_all_results(values_readed):
    entrycount = 1
    offsetx = 0
    offsety = 0
    ...
    for iti1 in range(1, 10):
        ...
        for iti2 in range(0, 9):
            ...
            widgetvalue = values_readed.get(entrycount)
            if widgetvalue == 0:
                return False

            if not check_sudoku_rule(widgetvalue, posx, posy):
                return False
                        ...
                        for value in exclusive_pairs[key1]:
                            for index_to_clean in range(0, 9):
                                ...
                                if value in possible_values_square[square][index_to_clean]:
                                    ...
                                    line_to_clean = int(((case_to_clean - (square * 9)) - 1) / 3) + 1 + (
                                                int(square / 3) * 3)
